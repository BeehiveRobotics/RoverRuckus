...
class RelicRecoveryTeleOp(): BROpMode() {
    ...
    override fun run() {
        ...
        if (Math.abs(controller1.leftStickX()) + Math.abs(controller1.leftStickY()) + Math.abs(controller1.rightStickX()) + Math.abs(controller1.rightStickY()) > Math.abs(controller2.leftStickX()) + Math.abs(controller2.leftStickY()) + Math.abs(controller2.rightStickX()) + Math.abs(controller2.rightStickY())) {
            ...
            if (controller1.dpad_up()) { //Forward
                robot.drive.forward(robot.D_PAD_SLOW_SPEED);
            } else if (controller1.dpad_left()) { //Right
                robot.drive.strafeRight(robot.D_PAD_SLOW_SPEED);
            } else if (controller1.dpad_down()) { //Backward
                robot.drive.backward(robot.D_PAD_SLOW_SPEED);
            } else if (controller1.dpad_right()) { //Left
                robot.drive.strafeLeft(robot.D_PAD_SLOW_SPEED);
            } else if (controller2.dpad_left()) { //Left
                robot.drive.strafeLeft(robot.D_PAD_SLOW_SPEED);
            } else if (controller2.dpad_right()) { //Right
                robot.drive.strafeRight(robot.D_PAD_SLOW_SPEED);
            } else {
